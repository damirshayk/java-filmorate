...
- `friendships` хранит направленную заявку в друзья и её статус: `PENDING` или `CONFIRMED`.

При принятии заявки в `friendships` обновляется статус уже существующей записи, а встречная запись для той же пары пользователей не создаётся.
...
Получение общих подтверждённых друзей двух пользователей:

```sql
WITH confirmed_friends AS (
...
SELECT u.user_id,
       ...
       u.birthday
FROM users AS u
JOIN confirmed_friends AS first_user
  ON first_user.friend_id = u.user_id
 AND first_user.user_id = :user_id
JOIN confirmed_friends AS second_user
  ON second_user.friend_id = u.user_id
 AND second_user.user_id = :other_user_id
ORDER BY u.user_id;
```
